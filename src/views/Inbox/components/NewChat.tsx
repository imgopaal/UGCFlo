...
import { GetContactsResponse, UserDetails } from '../types'
...
const NewChat = () => {
	...
	const { data, trigger: fetchContacts } = useSWRMutation(`/api/contacts/`, getContacts)
	useEffect(() => {
	...
	const handleSetSelectedContact = (contact: UserDetails) => {
		setSelectedContact(prevSelectedContacts => {
			const contactExists = prevSelectedContacts.some(c => c.id === contact.id)
	...
	return (
		<>
			<Button block variant="solid" onClick={() => setContactListDialog(true)}>
				New chat
			</Button>
			<Dialog isOpen={contactListDialog} onClose={handleDialogClose} onRequestClose={handleDialogClose}>
				{contacts && (
					<div>
						<div className="text-center mb-6">
							<h4 className="mb-1">Contact List</h4>
							<p>Browse and select contacts to start a conversation</p>
						</div>
						<DebouceInput
							placeholder="Search..."
							type="text"
						...
						/>
						<div className="mt-4">
							<p className="font-semibold uppercase text-xs mb-4">{contacts.length} person available</p>
							<div className="mb-6">
										...
										{contacts.map(contact => (
											<div
												...
												className={classNames(
													...
													selectedContact.some(item => item.id === contact.id) &&
											...
											>
												<div className="flex items-center gap-2">
													...
													<div>
														<p className="heading-text font-bold">{contact.name}</p>
														<p>{contact.email}</p>
													</div>
												</div>
												{selectedContact.some(item => item.id === contact.id) && (
							...
							<Button block variant="solid" onClick={handleStartNewChat}>
								{selectedContact.length > 1 ? 'Group Message' : 'Message'}
							</Button>
						</div>
					</div>
				)}
