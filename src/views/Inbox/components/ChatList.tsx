...
import { TbSearch, TbVolumeOff, TbX } from 'react-icons/tb'
							...
							.map(item => (
								<div
									...
									className={classNames(
										...
										selectedChat.id === item.id && 'bg-gray-100 dark:bg-gray-700'
									)}
									role="button"
									onClick={() =>
										handleChatClick({
								...
								>
										...
										<div className="min-w-0 flex-1">
											<div className="flex justify-between">
												<div className="font-bold heading-text truncate flex gap-2 items-center">
													<span>{item.name}</span>
													{item.muted && <TbVolumeOff className="opacity-60" />}
												</div>
											</div>
											<div className="truncate">{item.lastConversation}</div>
										</div>
									</div>
									<div className="flex gap-1 items-center">
										...
										{item.unread > 0 && <Badge className="bg-primary" />}
									</div>
								</div>
							))}
					</div>
